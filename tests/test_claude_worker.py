...
def make_stream_event(text: str):
    """Create a mock StreamEvent with a text_delta."""
...
class TestRunClaudeTurnStreaming:
    ...
    @pytest.mark.asyncio
    async def test_calls_append_text_for_each_stream_event(self):
        """_run_claude_turn_streaming calls manager.append_text() for each StreamEvent text delta."""
        from src.claude_worker import _run_claude_turn_streaming
    ...
    async def test_returns_concatenated_text(self):
        """_run_claude_turn_streaming returns concatenated text from StreamEvent deltas."""
        from src.claude_worker import _run_claude_turn_streaming
...
def _patch_simple_path():
    """Patches for simple path (no card_id) tests."""
    return (
...
def _patch_streaming_path(manager_mock=None):
    """Patches for streaming path (with card_id) tests."""
...
class TestSingleTurnWorker:
    """Tests for single_turn_worker — simple path (no card_id)."""
    ...
    async def test_worker_calls_query_with_prompt(self):
        ...
        with (
            ...
            await single_turn_worker(
                session=session, prompt="test prompt", reply_message_id="msg_reply_001",
                api_client=api_client, semaphore=semaphore, timeout=30.0,
            )
    ...
    async def test_worker_calls_update_card_on_success(self):
        ...
        with (
            patch("src.claude_worker.update_card_content", new=mock_update),
            patch("src.claude_worker.send_error_card", new=AsyncMock()),
        ):
            await single_turn_worker(
                session=session, prompt="test prompt", reply_message_id="msg_reply_001",
    ...
    async def test_worker_timeout_sends_error_card(self):
        ...
        with (
            patch("src.claude_worker.update_card_content", new=AsyncMock()),
            patch("src.claude_worker.send_error_card", new=mock_error),
        ):
            await single_turn_worker(
                session=session, prompt="slow prompt", reply_message_id="msg_reply_001",
                api_client=api_client, semaphore=semaphore, timeout=0.05,
            )
    ...
    async def test_worker_exception_sends_error_card(self):
        ...
        with (
            ...
            await single_turn_worker(
                session=session, prompt="bad prompt", reply_message_id="msg_reply_001",
                api_client=api_client, semaphore=semaphore, timeout=30.0,
            )
    ...
    async def test_worker_exception_does_not_propagate(self):
        ...
        with (
            ...
            # Should NOT raise
            await single_turn_worker(
                session=session, prompt="crashing prompt", reply_message_id="msg_reply_001",
                api_client=api_client, semaphore=semaphore, timeout=30.0,
            )

    @pytest.mark.asyncio
    async def test_worker_acquires_semaphore_then_lock(self):
        """Verify semaphore OUTER, session.lock INNER (Pitfall 4)."""
        from src.claude_worker import single_turn_worker
        ...
        task = asyncio.create_task(
            single_turn_worker(
                session=session, prompt="test", reply_message_id="msg_001",
                api_client=api_client, semaphore=semaphore, timeout=30.0,
            )
        )
    ...
    async def test_worker_uses_asyncio_wait_for(self):
        """Worker uses asyncio.wait_for for timeout."""
        from src.claude_worker import single_turn_worker
        ...
        with (
            ...
            await single_turn_worker(
                session=session, prompt="test", reply_message_id="msg_reply_001",
                api_client=api_client, semaphore=semaphore, timeout=0.05,
            )
    ...
    async def test_worker_error_card_send_failure_does_not_propagate(self):
        ...
        with (
            patch("src.claude_worker.update_card_content", new=AsyncMock()),
            patch("src.claude_worker.send_error_card", new=AsyncMock(side_effect=RuntimeError("card send failed"))),
        ):
...
class TestStreamingWorkerCardManager:
    """Tests for single_turn_worker streaming path (with card_id parameter)."""
    ...
    def _make_manager_mock(self):
        """Create a mock CardStreamingManager with _finalized tracking."""
        manager = MagicMock()
        ...
        async def _finalize_side_effect(*args, **kwargs):
            manager._finalized = True

        manager.finalize = AsyncMock(side_effect=_finalize_side_effect)
        return manager

    @pytest.mark.asyncio
    async def test_streaming_worker_creates_manager_with_card_id(self):
        """single_turn_worker creates CardStreamingManager with card_id and tenant_token."""
        from src.claude_worker import single_turn_worker
        ...
        manager_class_mock.assert_called_once()
        call_kwargs = manager_class_mock.call_args[1]
        assert call_kwargs["card_id"] == "card_123"
        assert call_kwargs["tenant_token"] == "token_test"
    ...
    async def test_streaming_worker_calls_finalize_on_success(self):
        """single_turn_worker calls manager.finalize() on success."""
        from src.claude_worker import single_turn_worker

        client, session, semaphore, api_client = self._make_streaming_mocks(response_items=[
            make_stream_event("Hello Claude"),
            make_result_message(),
        ])
        manager_mock = self._make_manager_mock()
    ...
    async def test_streaming_worker_calls_finalize_on_exception(self):
        ...
        with (
            ...
            await single_turn_worker(
                session=session, prompt="bad", reply_message_id="msg_002",
                api_client=api_client, semaphore=semaphore, timeout=30.0,
                card_id="card_002",
            )
    ...
    async def test_streaming_worker_calls_finalize_on_timeout(self):
        ...
        with (
            ...
            await single_turn_worker(
                session=session, prompt="slow", reply_message_id="msg_003",
                api_client=api_client, semaphore=semaphore, timeout=0.05,
                card_id="card_003",
            )
